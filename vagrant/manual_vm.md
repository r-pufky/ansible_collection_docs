...
requiring manual steps). Explicit reasons for using this test framework must be
clearly documented in `molecule.yml`.

These cannot be automated and should always be flag protected run. Test should
...
Confirm keys are set correctly:
...
Register VM - VMs must be registered to start in VirtualBox.
