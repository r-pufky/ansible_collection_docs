...
<> - Required element
{} - Lookup element
  - UPPER SPACES - docstring definitions
  - UPPER_WITH_UNDER - variables



### Docstring: `Single Line Defaults`
Simple variable comment block constrainted to 80 characters. Always use
[multiline-defaults](#multiline-defaults) instead of removing context.

Template
``` yaml
# [Enable|Disable|{TYPE}] {DESCRIPTION} [({UNIT})] [{CONTEXT}]<.?> Default: {VALUE} [({CONTEXT})].
{ROLE}_[service|config]_[section]_<transposed source variable>: {VALUE}
```
* Standard sentence case applies; uppercase abbreviations and proper nouns.
* Boolean specify flag meaning with [Enable|Disable] and end with ?

Example
``` yaml
# Enable search? Default: True.
role_enable_search: true

# Max download rate on program start (KB/sec) (0 no limit). Default: 0.
role_download_rate: 0

# List of allowed request origins. Default: ['*'] (all origins).
role_allowed_request_origins: ['*']
```

### Docstring: `<ultiline Defaults`
Complex variable comment block constrained to 80 characters.

Template
``` yaml
# [Enable|Disable|{TYPE}] {DESCRIPTION} [({UNIT})] [{CONTEXT}]<.?>
#
# {REQUIRES}.
#
